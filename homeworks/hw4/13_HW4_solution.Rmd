...
```{r}
# Plot the residuals for this regression 
# reg1_rw <- felm(residential ~ treated | 0 | 0 | state, data = data[industry_sector == "Total Electric Industry"])
...
# Observe that before regulations started happening, states that were ever deregulated systematically have lower than 0 errors, 
# suggesting a selection problem where states with higher prices were selected into treatment.  
ggplot(dt) + 
  geom_point(aes(year, residuals, colour =state))

# What does the implied model look like
dt[, residential_predict := reg1_rw$fitted.values]
ggplot(dt[state %in% c('GA','NY',"PA","AL")]) +
  geom_point(aes(year, residential, colour = state))  +
  geom_line(aes(year, residential_predict, colour = state))
```
...
```{r}
# We can also recover the state fixed effects
statefe <- as.data.table(getfe(reg2_rw))
statefe[,state := idx]
# Merge it into the data table, as well as a treatment effect
dt <- merge(dt, statefe, by = "state")
dt[, treatment_effect := treated*reg2_rw$coefficients[1,1]]
# The calculated residuals and the model residuals are 'different' because of floating point error
# https://stackoverflow.com/questions/2100490/floating-point-inaccuracy-examples
# https://blog.penjee.com/famous-number-computing-errors/  Rocket explosions as promised
dt[,residuals := residential - treatment_effect - effect]
dt[, residuals2 := reg2_rw$residuals]
dt[!residuals == residuals2,.(residuals, residuals2)]

# Residuals aren't random around 0, they exhibit differential trends that vary across states that we wrongly would attribute to deregulation
ggplot(dt) +
  geom_point(aes(year, residuals, colour =ever_dereg))

# What does the implied model look like for a couple states?
# Observe that GA, NY, PA and AL all have similar trends conditional on the starting point. 
# But in NY and PA, we (wrongfully) attribute this to deregulation
dt[, residential_predict := reg2_rw$fitted.values]
ggplot(dt[state %in% c('GA','NY',"PA","AL")]) +
  geom_point(aes(year, residential, colour = state))  +
  geom_line(aes(year, residential_predict, colour = state))

# Plot the same graph NET of fixed effects. 
ggplot(dt[state %in% c('GA','NY',"PA","AL")]) +
  geom_point(aes(year, residential - effect, colour = state))  +
  geom_line(aes(year, residential_predict - effect, colour = state))
```
...
```{r}
# Residuals now look mostly random-ish about 0, except for a couple of states
dt[, residuals := reg3_rw$residuals]
ggplot(dt) +
  geom_point(aes(year, residuals, colour =ever_dereg))
...
# Observe that GA, NY, PA and AL all have similar trends conditional on the starting point. 
# But in NY and PA, we (wrongfully) attribute this to deregulation
dt[, residential_predict := reg3_rw$fitted.values]
ggplot(dt[state %in% c('GA','NY',"PA","AL")]) +
  geom_point(aes(year, residential, colour = state))  +
  geom_line(aes(year, residential_predict, colour = state))
...
```{r}
# Get the state and year fixed effects
statefe <- as.data.table(getfe(reg3_rw))[fe=='state']
# Rename columns
statefe[, state :=idx]
statefe[, state_effect :=effect]
# Filter to just state and stateeffect
statefe<-statefe[, .(state, state_effect)]

# Get the year fixed effects
yearfe <- as.data.table(getfe(reg3_rw))[fe=='as.factor(year)']
# Rename columns
yearfe[, year :=as.numeric(as.character(idx))]
yearfe[, year_effect :=effect]

# Merge both into the data table
dt <- merge(dt, statefe, 'state')
dt <- merge(dt, yearfe, 'year')

# What does the implied model look like
ggplot(dt[state %in% c('NY','GA')]) +
  geom_point(aes(year, residential, colour = state))  +
  geom_line(aes(year, state_effect, colour = state)) +
  geom_line(aes(year, year_effect + state_effect, colour = state))+
  geom_line(aes(year, year_effect + state_effect + treated*reg3_rw$coefficients[1,1], colour = state)) 
#  geom_line(aes(year, 10+10*as.numeric(treated), colour = 'treatment_effect'))
